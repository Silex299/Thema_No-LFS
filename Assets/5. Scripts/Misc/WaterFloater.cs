...
namespace Misc
{
    public class WaterFloater : MonoBehaviour
    {
        ...
        void Float()
        {
            ...
            float rotationX = Mathf.PerlinNoise(time * frequency, 0f) * 2f - 1f; // Perlin noise returns 0..1, adjust to -1..1
            float rotationZ = Mathf.PerlinNoise(0f, time * frequency) * 2f - 1f; // Different axis for varied effect
            Quaternion newRotation = Quaternion.Euler(rotationX * rotAmplitude * 10f, startRot.eulerAngles.y, rotationZ * rotAmplitude * 10f);
